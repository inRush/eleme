...
export default {
  ...
  data() {
    ...
    const that = this;
    /** 所有组件定义的props名称数组 */
    const propsKeys = Object.keys((that.$options.props) || {});
  ...
  created() {
    ...
    const unwatchPropsFnArr = [];
    /** 所有 取消data的watch监听函数 的数组 */
    const unwatchDataFnArr = [];
    /** 所有组件定义的props名称数组 */
    const propsKeys = Object.keys((that.$options.props) || {});
    propsKeys.forEach((prop) => {
      const dataName = getDataName(prop);
      ...
      const propsFn = that.$watch(prop, (newVal, oldVal) => {
